...
def main():
	...
	for i in range(2 * window_size):
		...
		if stacked_embedings is None:
			...
			stacked_embedings = tf.concat(axis=2,
			                              values=[stacked_embedings, tf.reshape(embeding_i, [x_size, y_size, 1])])
	...
	for word, w_count in count:
		...
		unigrams[w_idx] = w_count * 1.0 / len(words)
	...
	candidate_sampler = tf.nn.fixed_unigram_candidate_sampler(true_classes=tf.cast(train_labels, dtype=tf.int64), num_true=1,
	                                      num_sampled=num_sampled, unique=True, range_max=vocabulary_size,
	                                      distortion=0.75, num_reserved_ids=0, unigrams=unigrams,
	...
	loss = tf.reduce_mean(
		tf.nn.sampled_softmax_loss(weights=softmax_weights, biases=softmax_biases, inputs=mean_embeddings,
		                           labels=train_labels,
		                           num_sampled=num_sampled, num_classes=vocabulary_size,sampled_values=candidate_sampler))
	...
	np.save('cbow_unigram', cbow_final_embeddings)
